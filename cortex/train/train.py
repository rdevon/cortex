from cortex.handlers.LossHandler import LossHandler
from cortex.utils import bad_values, update_dict_of_lists, viz, convert_to_numpy
from cortex.utils.viz import VizHandler, plot
...
from cortex import data, experiment, models
from cortex.train import reg, optimizer
...
_args_help = dict(
    ...
    test_mode='Testing mode. No train.',
    ...
    eval_during_train='Gives results over a train epoch.'
)
...
def train_on_routine(
    ...
    # Set requires grad from train models.
...
def perform_routine(
    ...
    # Run routine
    routine_results = {}
    if experiment.DEVICE == torch.device('cpu'):
        routine(
            data.DATA_HANDLER,
        ...
        with torch.cuda.device(experiment.DEVICE.index):
            routine(
                data.DATA_HANDLER,
                models.MODEL_HANDLER,
                loss_handler,
...
def train_epoch(epoch, viz_handler, quit_on_bad_values, eval_during_train):
    ...
    active_loss_models = list(
        set([m for ms in ROUTINE_MODELS.values() for m in ms]))
    ...
    is_training = ', '.join(
        [k for k in num_updates_dict.keys() if num_updates_dict[k] > 0])
    data.DATA_HANDLER.reset(
        string='Training (epoch {}) ({}): '.format(
            epoch, is_training))
    viz_handler.ignore = True
    routine_args = experiment.ARGS['routines']
    ...
    try:
        while True:
            ...
            for routine_key, routine in models.ARCH.train_routines.items():
                ...
                routine_losses.update(**dict((k, v.item())
...
def test_epoch(epoch, viz_handler, eval_mode=False, test=True, viz=True):
    ...
    if test:
        routine_args = experiment.ARGS['test_routines']
    else:
        routine_args = experiment.ARGS['routines']
...
def display_results(
        ...
        total_time):
    if epochs and epoch:
        print('\n\tEpoch {}/{} took {:.3f}s. Total time: {:.2f}'.format(epoch +
                                                                        1, epochs, epoch_time, total_time))
    ...
    for k in train_results.keys():
        ...
        if v_test is None:
            ...
            if isinstance(v_train, dict):
                print('\t' + k + ': ' + ' | '.join(['{}: {:.2f} / {:.2f}'.format(
                    k_, v_train[k_], v_test[k_]) for k_ in v_train.keys()]))
            else:
        ...
        if k in d_train and k in d_test:
            ...
            if isinstance(v_train, dict):
                ...
                for k_, v in v_train.items():
                    if len(v) < max_len:
                        v_train[k_] = v_train[k_] + [v_train[k_]
                                                     [-1]] * (max_len - len(v_train[k_]))
...
def main_loop(
        ...
        save_on_highest=None,
        eval_during_train=True):
    info = pprint.pformat(experiment.ARGS)
    viz.visualizer.text(info, env=experiment.NAME, win='info')
    vh = VizHandler()
    total_time = 0.
    if test_mode:
    ...
    try:
        for e in range(epochs):
            epoch = experiment.INFO['epoch']

            start_time = time.time()

            # TRAINING
            train_results_ = train_epoch(
                epoch, vh, quit_on_bad_values, eval_during_train)
            convert_to_numpy(train_results_)
            update_dict_of_lists(experiment.SUMMARY['train'], **train_results_)
                    ...
                    if found_best:
                        ...
                        print(
                            '\nFound best {} (train): {}'.format(
                                save_on_best, best))
                        experiment.save(prefix='best_' + save_on_best)

            # TESTING
            test_results_ = test_epoch(epoch, vh)
            convert_to_numpy(test_results_)
            update_dict_of_lists(experiment.SUMMARY['test'], **test_results_)

            align_summaries(experiment.SUMMARY['train'], experiment.SUMMARY['test'])
            ...
            if (archive_every and epoch % archive_every == 0):
                experiment.save(prefix=epoch)

            experiment.INFO['epoch'] += 1
        ...
        if kill:
            ...
            experiment.save(prefix='interrupted')
            sys.exit(0)

    LOGGER.info('Successfully completed train')
    experiment.save(prefix='final')
