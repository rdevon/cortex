...
DEFAULTS = dict(
    ...
    model=dict(
        discriminator_args=discriminator_args_, generator_args=generator_args_),
    procedures=dict(
        measure='gan', penalty=5.0, n_samples=10, penalty_type='gradient_norm'),
    test_procedures=dict(n_samples=100),
    train=dict(epochs=200, summary_updates=100, archive_every=10))
...
def setup(data=None,
          ...
          train=None):
    data['noise_variables']['r'] = ('uniform',
                                    procedures['n_samples'] * 28 * 28 * 1)
    data['noise_variables']['r_t'] = (
        'uniform', test_procedures['n_samples'] * 28 * 28 * 1)


def discrete_gan(nets,
                 inputs,
                 measure=None,
                 penalty=None,
                 n_samples=10,
    ...
    if R.size()[1] != DIM_C * n_samples * DIM_X * DIM_Y:
        R = inputs['r_t']
    assert R.size() == (B, DIM_C * n_samples * DIM_X * DIM_Y), (R.size(), (
        B, DIM_C * n_samples * DIM_X * DIM_Y))
    ...
    if (measure == 'w' and not test_mode) or use_sm:
        ...
        d_loss, g_loss, r, f, w, b = f_divergence(measure, real_out,
                                                  fake_out_sm)
    else:
        d_loss, g_loss, r, f, w, b = f_divergence(measure, real_out,
                                                  fake_out.view(M, B, -1))

    if measure in ('gan', 'jsd', 'rkl', 'kl', 'sh', 'proxy_gan',
                   'dv') and not use_sm:
    ...
    results = dict(
        ...
        distance=-d_loss.data[0],
        boundary=torch.mean(b).data[0],
        real=torch.mean(r).data[0],
        fake=torch.mean(f).data[0],
        gen_out=g_output.mean().data[0],
        w_tilde=w_tilde.mean().data[0],
        real_out=real_out.mean().data[0],
        fake_out=fake_out.mean().data[0])

    if measure != 'w' and not use_sm:
        results.update(
            alpha=alpha.mean().data[0],
            log_alpha=log_alpha.mean().data[0],
            beta=beta.mean().data[0],
            log_beta=log_beta.mean().data[0])
        results.update(ess=(1. / (w_tilde**2).sum(0)).mean().data[0])

    if test_mode or measure == 'w' or use_sm:
        fake_out_sm = discriminator(
            Variable(g_output.data.cuda(), volatile=True))
        ...
        results.update(
            distance_th=dist_th.data[0],
            distance_sam=dist_sam.data[0],
            distance_sm=dist_sm.data[0])

    samples = dict(
        images=dict(generated=gen_out.data, prob=g_output.data, real=X.data))

    if penalty:
        p_term = apply_penalty(
            inputs,
            discriminator,
            X,
            g_output,
    ...
    results.update(
        log_Z=log_Z.data[0],
        log_Z_est=log_Z_est.mean().data[0],
        log_w=log_w.mean().data[0],
        log_g=log_g.mean().data[0])
    return dict(
        generator=g_loss, discriminator=d_loss), results, samples, 'boundary'
