...
mnist_encoder_args_ = dict(
    dim_h=64, batch_norm=True, f_size=5, pad=2, stride=2, min_dim=7)
...
def update_encoder_args(x_shape, model_type='convnet', encoder_args=None):
    ...
    if model_type == 'resnet':
        ...
        if len(model_attributes) != 2:
            raise ValueError('`tv-wrapper` model type should be in form'
                             ' `tv-wrapper.<MODEL>`')
        model_key = model_attributes[1]
...
mnist_decoder_args_ = dict(
    dim_h=64, batch_norm=True, f_size=4, pad=1, stride=2, n_steps=2)
...
def update_decoder_args(x_shape, model_type='convnet', decoder_args=None):
    ...
    if model_type == 'resnet':
        ...
        from cortex.built_ins.networks.conv_decoders import (SimpleConvDecoder
                                                             as Decoder)
