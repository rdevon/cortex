...
def ms_ssim(X_a, X_b, window_size=11,
            size_average=True, C1=0.01**2, C2=0.03**2):
    '''
    ...
    mu1 = torch.nn.functional.conv2d(
        X_a, window, padding=window_size // 2, groups=channel)
    mu2 = torch.nn.functional.conv2d(
        X_b, window, padding=window_size // 2, groups=channel)
...
def update_encoder_args(x_shape, model_type='convnet', encoder_args=None):
    ...
    if model_type == 'resnet':
        ...
        # and dim_out.
        Encoder = lambda shape, dim_out=None, n_steps=None, **kwargs: tv_model(
            num_classes=dim_out, **kwargs)
        encoder_args_ = {}
        ...
        if len(model_attributes) != 2:
            raise ValueError(
                '`tv-wrapper` model type should be in form `tv-wrapper.<MODEL>`')
        model_key = model_attributes[1]
...
def update_decoder_args(x_shape, model_type='convnet', decoder_args=None):
    ...
    if model_type == 'resnet':
        ...
        from cortex.built_ins.networks.conv_decoders import SimpleConvDecoder as Decoder
