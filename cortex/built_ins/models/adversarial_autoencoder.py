...
def penalty_routine(data, models, losses, results, viz, penalty_amount=0.5,
                    output_nonlin=None, noise_type=None):
    X, Z_P, U = data.get_batch('images', 'y', 'u')
    Z_P = shape_noise(Z_P, U, noise_type)
...
def BUILD(data, models, encoder_type='convnet', decoder_type='convnet', dim_z=64,
    ...
    encoder = build_encoder(None, x_shape, dim_z, Encoder,
...
TRAIN_ROUTINES = dict(discriminator=discriminator_routine,
                      autoencoder=main_routine, classifier=classifier_routine)
