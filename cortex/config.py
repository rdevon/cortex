...
def configure():
    torchvisionDataPath = filedialog.askdirectory(
        title="Select torchvision data path.")
    dataPathsLoopSize = input(
        "Please enter the number of data paths you want to add: ")
    dataPaths = dict()
    for x in range(int(dataPathsLoopSize)):
        datasetName = input(
            "Please enter dataset {} name (i.e.: CelebA): ".format(x + 1))
        dataPath = filedialog.askdirectory(
            title="Select data path {}.".format(x + 1))
