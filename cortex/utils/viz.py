...
'''
from cortex.utils import convert_to_numpy, compute_tsne
from cortex.utils.viz_utils import tile_raster_images
...
from cortex import data, experiment
...
class VizHandler():
    def __init__(self):
        self.clear()
        self.ignore = True
        self.image_dir = experiment.OUT_DIRS.get('image_dir', None)
        self.prefix = experiment._file_string('')
...
def plot():
    ...
    train_summary = experiment.SUMMARY['train']
    test_summary = experiment.SUMMARY['test']
    for k in train_summary.keys():
        ...
        visualizer.line(Y=Y, X=X, env=experiment.NAME, opts=opts, win='line_{}'.format(k))
...
def save_heatmap(X, out_file=None, caption='', title='', image_id=0):
    visualizer.heatmap(X=X, opts=dict(title=title, caption=caption), win='heatmap_{}'.format(image_id), env=experiment.NAME)
...
def save_scatter(points, out_file=None, labels=None, caption='', title='', image_id=0):
    ...
    visualizer.scatter(X=points, Y=Y, opts=dict(title=title, caption=caption, legend=names, markersize=5),
                       win='scatter_{}'.format(image_id), env=experiment.NAME)
...
def save_movie(images, num_x, num_y, out_file=None, movie_id=0):
    ...
    visualizer.video(videofile=out_file, env=experiment.NAME,
