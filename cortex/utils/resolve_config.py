# -*- coding: utf-8 -*-
"""
:mod:`cortex.core.resolve_config` -- Configuration parsing and resolving
=======================================================================

.. module:: resolve_config
   :platform: Unix
   :synopsis: How does cortex resolve configuration settings?

How:

 - Experiment name resolves like this:
    * cmd-arg **>** cmd-provided cortex_config **>** REQUIRED (no default is given)

 - Database options resolve with the following precedence (high to low):
    * cmd-provided cortex_config **>** env vars **>** default files **>** defaults
...
.. seealso:: :const:`ENV_VARS`, :const:`ENV_VARS_DB`
 ...
 - All other managerial, `Optimization` or `Dynamic` options resolve like this:
    ...
    * cmd-args **>** cmd-provided cortex_config **>** database (if experiment name
 ...
 * default cortex example file **<** system-wide config **<** user-wide config
...
DEF_CONFIG_FILES_PATHS = [
    os.path.join(cortex.core.DIRS.site_data_dir, 'cortex_config.yaml.example'),
    os.path.join(cortex.core.DIRS.site_config_dir, 'cortex_config.yaml'),
    os.path.join(cortex.core.DIRS.user_config_dir, 'cortex_config.yaml')
    ]

# list containing tuples of
# (environmental variable names, configuration keys, default values)
ENV_VARS_DB = [
    ('CORTEX_DB_NAME', 'name', 'cortex'),
    ('CORTEX_DB_TYPE', 'type', 'MongoDB'),
    ('CORTEX_DB_ADDRESS', 'host', socket.gethostbyname(socket.gethostname()))
    ]
...
def fetch_cortex_args(description):
    """Get options from command line arguments.

    :param description: string description of ``cortex`` executable
    ...
    parser.add_argument(
        '-V', '--version',
        action='version', version='cortex ' + cortex.core.__version__)
    ...
    cortex_group = parser.add_argument_group(
        ...
        description="These arguments determine cortex's behaviour")
    ...
    cortex_group.add_argument(
        ...
        help="user provided cortex configuration file")
    ...
    usergroup = parser.add_argument_group(
        ...
        description="These arguments determine user's script behaviour "
                    "and they can serve as cortex's parameter declaration.")
    ...
    # Explicitly add cortex's version as experiment's metadata
    args['metadata'] = dict()
    args['metadata']['cortex_version'] = cortex.core.__version__
    log.debug("Using cortex version %s", args['metadata']['cortex_version'])

    cortex_file = args.pop('config')
    config = dict()
    if cortex_file:
        log.debug("Found cortex configuration file at: %s", os.path.abspath(cortex_file.name))
        config = yaml.safe_load(cortex_file)
...
def merge_env_vars(config):
    """Fetch environmental variables related to cortex's managerial data.
...
def merge_cortex_config(config, dbconfig, cmdconfig, cmdargs):
    """
    Oríon Configuration
    -------------------
    ...
    name --  Experiment's name.
       ...
       database which are not overwritten by current call to `cortex` script.

    max_trials -- Maximum number of trial evaluations to be computed

       (required as a cmd line argument or a cortex_config parameter)
