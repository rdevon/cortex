...
from cortex.nets.ali import build_extra_networks
from cortex.nets.gan import get_positive_expectation, get_negative_expectation, generator_loss, apply_gradient_penalty
from cortex.nets.utils import perform_svc, update_encoder_args, update_decoder_args
...
def visualize(Z_Q, P_samples, Q_samples, X, T, Y_Q=None, viz=None, scatter=False):
    if viz is not None:
...
def penalty_routine(data, models, losses, results, viz, penalty_amount=0.2, output_nonlin=None, noise_type=None):
    X, Y_P, U = data.get_batch('images', 'y', 'u')
    Y_P = shape_noise(Y_P, U, noise_type)
    Z_P, Z_Q, Y_Q = encode(models, X, Y_P, output_nonlin=output_nonlin, noise_type=noise_type)
...
INFO = dict(measure=dict(choices=['GAN', 'JSD', 'KL', 'RKL', 'X2', 'H2', 'DV', 'W1'],
            ...
            add_supervision=dict(help='Use additional nets for monitoring during train.')
)
