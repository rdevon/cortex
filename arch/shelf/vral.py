...
from featnet import build_discriminator, score
from vae import update_encoder_args, update_decoder_args


def encoder_routine(data, models, losses, results, viz, measure=None, 
    ...
    viz.add_histogram(dict(fake=W_Q.view(-1).data, real=W_P.view(-1).data),
                      name='encoder output')
...
def discriminator_routine(data, models, losses, results, viz,
                          measure='GAN', offset=1.0):
    ...
    # Real discriminator
    E_P_pos, E_P_neg, S_PP, S_PQ = score(models, Y_P, W_P, measure,
                                         key='real_discriminator')
    P_difference = E_P_pos - E_P_neg

    # Fake discriminator
    E_Q_pos, E_Q_neg, S_QP, S_QQ = score(models, Y_Q, W_Q - offset, measure,
    ...
    results['{} distances'.format(measure)] = dict(
        P=P_difference.item(), Q=Q_difference.item())
    viz.add_image(X_P, name='ground truth')
    viz.add_histogram(dict(fake=S_PQ.view(-1).data, real=S_PP.view(-1).data),
                      name='real discriminator output')
    viz.add_histogram(dict(fake=S_QQ.view(-1).data, real=S_QP.view(-1).data),
                      name='fake discriminator output')

    losses.real_discriminator = -P_difference
    losses.fake_discriminator = -Q_difference


def penalty_routine(data, models, losses, results, viz,
                    penalty_amount=2.0, offset=None,
    ...
    P_penalty = apply_gradient_penalty(
        data, models, inputs=(Y_P, W_P - offset), model='real_discriminator',
        penalty_amount=penalty_amount)
    if P_penalty:
        losses.real_discriminator = P_penalty

    Q_penalty = apply_gradient_penalty(
        data, models, inputs=(Y_Q, W_Q), model='fake_discriminator',
        penalty_amount=penalty_amount)
    if Q_penalty:
        losses.fake_discriminator = Q_penalty

    E_penalty = apply_gradient_penalty(
        data, models, inputs=(X_P, X_Q),
