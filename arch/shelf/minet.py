...
from featnet import (apply_gradient_penalty, build_encoder,
                     build_discriminator as build_noise_discriminator, encode,
...
def encoder_routine(data, models, losses, results, viz, measure=None, noise_measure=None,
                    noise_type=None, output_nonlin=False, generator_loss_type='non-saturating',
                    beta=1.0, key='discriminator',
    ...
    if 'noise_discriminator' in models:
        ...
        E_pos_n, E_neg_n, P_samples_n, Q_samples_n = featnet_score(models, Z_P, Z, measure,
                                                                   Y_P=Y_P, Y_Q=Y_Q,
        ...
        losses.encoder += beta * generator_loss(Q_samples_n, noise_measure,
                                                loss_type=generator_loss_type)


def discriminator_routine(data, models, losses, results, viz, measure='JSD',
                          penalty_amount=0.5, output_nonlin=None, noise_type='hypercubes',
                          noise='uniform', correct_bias=False):
...
def noise_discriminator_routine(data, models, losses, results, viz,
                                noise_penalty_amount=0.5, noise_measure='JSD',
    ...
    E_pos, E_neg, _, _ = featnet_score(
        models, Z_P, Z_Q, noise_measure, Y_P=Y_P, Y_Q=Y_Q, key='noise_discriminator')
...
def BUILD(data, models, encoder_type='convnet', decoder_type='convnet',
          discriminator_type='convnet', dim_embedding=64, dim_noise=64, encoder_args={},
          decoder_args={}, discriminator_args={}, use_topnet=False, match_noise=False,
          noise_type=None, add_supervision=False):
    ...
    build_encoder(models, x_shape, dim_noise, Encoder,
                  fully_connected_layers=[1028], use_topnet=use_topnet,
...
INFO = dict(measure=dict(choices=['GAN', 'JSD', 'KL', 'RKL', 'X2', 'H2', 'DV', 'W1'],
            ...
            use_topnet=dict(help='Whether to use an additional network and perform ALI with top '
                            'two variables.'),
