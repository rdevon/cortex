...
def setup(model=None, data=None, procedures=None, **kwargs):
    ...
    data['noise_variables'] = dict(
        y=(noise, model['dim_noise']), z=(noise, 1), x=('normal', model['dim_in']))
...
def make_graph(nets, data_handler, measure=None, boundary_seek=False, penalty=None):
    ...
    d_loss, e_loss, rr, fr, wr, br = f_divergence(
        measure, real_r, real_f, boundary_seek=boundary_seek)

    log_ratio = -0.5 * ((X ** 2).sum(1) - float(X.size(1)) *
                        math.log(2. * math.pi)) - torch.log(Zf.clamp(1.e-5, 1.))
    ...
    if penalty:
        ...
        g_r = autograd.grad(outputs=real_out, inputs=real,
                            grad_outputs=torch.ones(real_out.size()).cuda(),
                            create_graph=True, retain_graph=True, only_inputs=True)[0]

        g_f = autograd.grad(outputs=fake_out, inputs=fake,
                            grad_outputs=torch.ones(fake_out.size()).cuda(),
                            create_graph=True, retain_graph=True, only_inputs=True)[0]
