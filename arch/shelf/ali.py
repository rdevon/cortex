...
def discriminator_routine(data, models, losses, results, viz, measure='GAN'):
    ...
    viz.add_histogram(dict(fake=Q_samples.view(-1).data, real=P_samples.view(-1).data),
...
def build_discriminator(models, x_shape, dim_z, Encoder, key='discriminator', **encoder_args):
    ...
    logger.debug(
        'Forming discriminator with class {} and args: {}'.format(Encoder, discriminator_args))
...
def build_extra_networks(models, x_shape, dim_z, dim_l, Decoder, dropout=0.1,
    ...
    classifier = FullyConnectedNet(dim_z, dim_h=[200, 200], dim_out=dim_l,
...
def BUILD(data, models, encoder_type='convnet', decoder_type='convnet', discriminator_type='convnet', dim_z=64,
    ...
    encoder = build_encoder(
        None, x_shape, dim_z, Encoder, fully_connected_layers=[1028], **encoder_args)
...
TRAIN_ROUTINES = dict(
    discriminator=discriminator_routine, penalty=penalty_routine, generator=generator_routine)
