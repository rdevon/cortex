...
def autoencoder(input_shape, n_filters, filter_sizes, t):
	...
	cifar10: input_shape=[None, 900], n_filters=[10, 10, 10], filter_sizes=[3, 3, 3], t=1
	...
	temp = encoder(input_shape, n_filters, filter_sizes, t)
	temp1 = decoder(temp['x'], temp['z'], temp['enc'], temp['shapes'], temp['x_tensor'])
	
	return {'x': temp['x'], 'z': temp['z'], 'y': temp1['y'], 'cost': temp1['cost'], 'summary_op': temp1['summary_op']}
	
	
	
	
def encoder(input_shape, n_filters, filter_sizes, t):
	...
	return{'x': x, 'z': z, 'enc': enc, 'shapes': shapes, 'x_tensor': x_tensor}
	

def decoder(x, z, enc, shapes, x_tensor):
	...
	return{'y' : y, 'cost': cost, 'summary_op': summary_op}
