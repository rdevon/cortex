...
class Classifier():
    ...
    def routine(self, data, models, losses, results, viz, key='classifier', criterion=None):
        classifier = models[key]
        inputs, targets = data.get_batch('images', 'targets')

        predicted = self.classify(classifier, inputs, targets, losses=losses, results=results, criterion=criterion, key=key)
