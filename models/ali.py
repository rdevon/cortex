...
DEFAULTS = dict(
    ...
    optimizer=dict(
        optimizer='Adam',
        learning_rate=1e-4,
        updates_per_model=dict(discriminator=1, nets=1, classifier=1)
    ),
...
def build_graph(nets, data_handler, measure=None, boundary_seek=False, penalty=None, penalty_type='gradient_norm'):
    ...
    S_r = topnet(torch.cat([W_r, Z_r], 1))
    W_f = discriminator(X_f)
    S_f = topnet(torch.cat([W_f, Z_f], 1))

    d_loss, g_loss, r, f, w, b = f_divergence(measure, S_r, S_f, boundary_seek=boundary_seek)

    # classification
    y_hat = classifier(Variable(Z_r.data.cuda(), requires_grad=False), nonlinearity=F.log_softmax)
    ...
    losses = dict(nets=g_loss, discriminator=d_loss, classifier=c_loss)
...
def build_model(data_handler, model_type='convnet', dim_z=64, encoder_args=None, decoder_args=None,
    ...
    discriminator_args_.update(full_connected_layers=[], batch_norm=False)
    ...
    return dict(discriminator=(discriminator, topnet), nets=(encoder, decoder), classifier=classifier), build_graph