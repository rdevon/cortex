...
mnist_encoder_args_ = dict(dim_h=64, batch_norm=True, f_size=5, pad=2, stride=2, min_dim=7, nonlinearity='LeakyReLU')
...
DEFAULTS = dict(
    data=dict(batch_size=dict(train=64, test=1028), skip_last_batch=True,
              noise_variables=dict(r=('normal', 16))),
    optimizer=dict(
        optimizer='Adam',
...
def train(nets, data_handler, measure=None, boundary_seek=False, penalty=None, r_lambda=1., e_lambda=1.,
          noise_scale=1.0):
    ...
    results = dict(e_loss=e_loss.data[0], r_loss=r_loss.data[0], d_loss=d_loss.data[0],
                   ...
                   real=Rf.mean().data[0], real_std=torch.std(Rf).data[0], accuracy=correct)
    ...
    if penalty:
        ...
        z_ = encoder(X_)
        g = autograd.grad(outputs=z_.sum(), inputs=X_, grad_outputs=torch.ones(z_.sum().size()).cuda(),
...
def build_model(data_handler, model_type='convnet', dim_d=16, dim_e=2, encoder_args=None, decoder_args=None):
    ...
    topnet = DenseNet(dim_e, dim_h=[64, 128, 64], dim_out=dim_d, batch_norm=True)
    revnet = DenseNet(dim_d, dim_h=[64, 128, 64], dim_out=dim_e, batch_norm=True)
    discriminator = DenseNet(dim_d+dim_e, dim_h=[64, 128, 128, 64], dim_out=1, nonlinearity='ReLU', batch_norm=False)
    logger.debug(discriminator)
    logger.debug(encoder)

    return dict(discriminator=discriminator, nets=[encoder, topnet, revnet], ss_nets=[decoder, classifier]), train
